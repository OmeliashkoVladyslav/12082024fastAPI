...
class BaseStorage(ABC):
    ...
    @abstractmethod
    def update_tour(self, tour_id: str, name_tour: str):
        pass
...
class JSONStorage(BaseStorage):
    ...
    def get_tours(self, skip: int = 0, limit: int = 10, search_param: str = ''):
        ...
        if search_param:
            ...
            for tour in content:
                if (
                        search_param in tour['name_tour'].lower()
                        or search_param in tour['title'].lower()
                        or search_param in tour['description'].lower()
                ):
                    data.append(tour)
    ...
    def get_tour_info(self, tour_id: str) -> dict:
        with open(self.__file_name, mode='r') as file:
            content: list[dict] = json.load(file)
        for tour in content:
            if tour_id == tour['id']:
    ...
    def get_tours_info(self, tours_ids: set[str]) -> list:
        tours = []
        with open(self.__file_name, mode='r') as file:
            content: list[dict] = json.load(file)
        for tour in content:
            if tour['id'] in tours_ids:
                tours.append(tour)
        return tours
